...
问题三在问题二容量硬约束方案已经确定的基础上研究服务定价。定价既影响老人价格满意度，也影响服务站收入、政府补贴和利润率。本文先给出定价变量、补贴约束和利润率约束，再给出临界点枚举求解过程，最后用分站财务表检验方案是否满足保本微利要求。
...
\end{equation}
这一候选集同时覆盖降价释放潜在需求、基准价格运营和小幅上浮维持利润三类情形。
...
\begin{enumerate}
    \item[Step 1] 固定问题二得到的站点位置、规模和容量承接结果。
    \item[Step 2] 对五类收费服务生成候选价格集合，紧急救助价格固定为 0。
    \item[Step 3] 对每个服务站枚举 $5^5$ 个价格组合，计算价格满意度和价格变化后的需求释放量。
    \item[Step 4] 在容量约束下重新核算实际服务量，计算服务收入、政府补贴、直接成本和运营成本。
    \item[Step 5] 删除利润率不满足 $0\leq \rho_j\leq 8\%$ 的价格组合。
    \item[Step 6] 在可行组合中选择价格满意度最高的方案；若价格满意度相同，则选择利润率更接近且不超过 $8\%$ 的方案。
\end{enumerate}
